...
function checkLoginStatus() {
    ...
    const userEmail = localStorage.getItem('eniac_user_email') || 'usuario@email.com';
    
    if (isLoggedIn) {
        showLoggedInState(userName, userEmail);
    } else {
        showLoginForm();
    }
}

function showLoggedInState(userName, userEmail) {
    ...
    // Hide login form and show logged in card
    ...
    // Update user info in logged in card
    if (loggedUserName) loggedUserName.textContent = userName;
    if (loggedUserEmail) loggedUserEmail.textContent = userEmail;
}
...
function showLoginForm() {
    ...
    // Show login form and hide logged in card
    if (loginCard) loginCard.style.display = 'block';
    if (loggedInCard) loggedInCard.style.display = 'none';
    
    // Hide greeting in header
    if (userGreeting) userGreeting.style.display = 'none';
}

function setLoggedInStatus(userName, userEmail) {
    localStorage.setItem('eniac_logged_in', 'true');
    localStorage.setItem('eniac_user_name', userName);
    localStorage.setItem('eniac_user_email', userEmail);
...
function initLoginForm() {
    ...
    loginForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            setLoggedInStatus(userName, email);
...
function showDemoHint() {
    ...
    if (emailInput && localStorage.getItem('eniac_logged_in') !== 'true') {
        emailInput.addEventListener('focus', function() {
            if (!this.value) {
                showNotification('💡 Digite qualquer email e senha para fazer login', 'info');
            }
        }, { once: true });
    }
}
...
// ===== GLOBAL LOGIN STATUS FUNCTIONS =====
// These functions can be used by other pages to check login status
window.isUserLoggedIn = function() {
    return localStorage.getItem('eniac_logged_in') === 'true';
};

window.getUserInfo = function() {
    return {
        name: localStorage.getItem('eniac_user_name') || 'Usuário',
        email: localStorage.getItem('eniac_user_email') || 'usuario@email.com'
    };
};
