// Permitir chamada global para onclick no HTML
...
function updateAuthUI() {
    ...
    if (authSection && isLoggedIn) {
        authSection.innerHTML = `
            ...
            <button onclick="logout()" class="logout-btn sidebar-logout-btn">Sair</button>
        `;
    }
}
...
function updateUserPointsDisplay() {
    // Sempre buscar pontos atualizados do backend
    fetch('/api/auth/me', { credentials: 'include' })
        ...
        .then(userData => {
            if (userData) {
                ...
                if (userPointsSpan) userPointsSpan.textContent = `Pontos: ${userPoints}`;
...
async function confirmRedeem() {
    ...
    try {
        ...
        const productPointsSpent = currentProduct.price;
        ...
        if (response.ok) {
            ...
            // Mostrar modal de sucesso usando variáveis salvas
            document.getElementById('successProductName').textContent = productName;
            document.getElementById('successPointsSpent').textContent = productPointsSpent.toLocaleString('pt-BR');
